...
it('can add the star name and star symbol properly', async() => {
    let instance = await StarNotary.deployed();
    instance.name = "StarName";
    instance.symbol = "CSD";
    assert.equal(instance.name, "StarName");
    assert.equal(instance.symbol, "CSD");
});
...
it('lets 2 users exchange stars', async() => {
    ...
    await instance.createStar('super awsome star', starId2, {from: user2});
...
it('lets a user transfer a star', async() => {
    ...
    let starId = 8;
    await instance.createStar('awesome star', starId, {from: user1});
    assert.equal(user1, await instance.ownerOf(starId));
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let user = accounts[1];
    let starId = 9;
    await instance.createStar('awesome star name', starId, {from: user});

    let starName = await instance.lookUptokenIdToStarInfo(starId);
    assert.equal("awesome star name", starName);
});